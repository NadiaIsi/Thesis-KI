...
Code regarding importing and sampling the supplier dataset has been hashed so the code will still run without this dataset.
"""
...
# export this sample to a csv for manual labelling.
...
#Jaccard index to calculate word similarity
...
if __name__ == '__main__':
    #data = cleanSupplier(data)
    cora = cleanCora(cora)
    res = cleanRes(res)
    # running the validating algorithms on the datasets or samples.
    #test1 = data[0:1000].reset_index()
    # supDup = validate_testset(test1) # this is one sample, multiple samples can be made from the datasets and tested in the same way.
    resDup = validate_resset(res)
    coraDup = validate_coraset(cora)
